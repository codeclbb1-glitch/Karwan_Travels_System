...
export const authApi = {
  ...
  profile: async () => { const { data, error } = await supabase.from("profiles").select("id, full_name, role").single(); if (error) throw error; return data as { id: string; full_name: string; role: Role }; },
};
